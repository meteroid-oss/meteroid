pub mod components {
    ...
    use meteroid_grpc::meteroid::api::components::v1_2 as api;
    use meteroid_store::domain::{price_components as domain};
    use tonic::Status;
    use uuid::Uuid;
    use meteroid_grpc::meteroid::api::components::v1_2::EditPriceComponentRequest;
    ...
    use crate::api::subscriptions::ext::{billing_type_from_grpc, billing_type_to_grpc, usage_pricing_model_from_grpc, usage_pricing_model_to_grpc};
    use meteroid_store::domain::enums::BillingType;
    ...
    pub fn edit_api_to_domain(
        ...
        let component = comp.component.ok_or(Status::invalid_argument("component is missing"))?;
    ...
    fn map_fee_to_domain(fee: Option<api::Fee>) -> Result<domain::FeeType, Status> {
        match fee.as_ref()
            .and_then(|fee| fee.fee_type.as_ref()) {
            Some(s) => {
                match s {
                    api::fee::FeeType::Rate(fee) => {
                        Ok::<_, Status>(domain::FeeType::Rate {
                            rates: fee.rates.iter().map(|rate| Ok::<_, Status>(domain::TermRate {
                                term: billing_period::from_proto(rate.term()),
                                price: Decimal::from_proto_ref(&rate.price)?,
                            })).collect::<Result<Vec<_>, _>>()?,
                        })
                    }
                    api::fee::FeeType::Slot(fee) => {
                        Ok(domain::FeeType::Slot {
                    ...
                    api::fee::FeeType::Capacity(fee) => {
                        Ok(domain::FeeType::Capacity {
                            metric_id: Uuid::from_proto_ref(&fee.metric_id)?,
                            thresholds: fee.thresholds.iter().map(|threshold| Ok::<_, Status>(domain::CapacityThreshold {
                                price: Decimal::from_proto_ref(&threshold.price)?,
                                per_unit_overage: Decimal::from_proto_ref(&threshold.per_unit_overage)?,
                                included_amount: threshold.included_amount,
                            })).collect::<Result<Vec<_>, _>>()?,
                        })
                    }
                    api::fee::FeeType::ExtraRecurring(fee) => {
                        let cadence = fee.term
                    ...
                    api::fee::FeeType::OneTime(fee) => {
                        Ok(domain::FeeType::OneTime {
                            quantity: fee.quantity,
                            unit_price: Decimal::from_proto_ref(&fee.unit_price)?,
                        })
                    }
    ...
    pub fn domain_to_api(
        comp: domain::PriceComponent,
    ) -> Result<api::PriceComponent, Status> {
    ...
    fn map_fee_domain_to_api(fee: domain::FeeType) -> api::Fee {
        let fee_type = match fee {
            domain::FeeType::Rate { rates } => {
                let rates = rates.into_iter().map(|rate| api::fee::TermRate {
                    term: billing_period::to_proto(rate.term).into(),
                    price: rate.price.as_proto(),
                }).collect();

                api::fee::FeeType::Rate(api::fee::RateFee {
                    rates,
                })
            }
            domain::FeeType::Slot {
            ...
            domain::FeeType::Capacity {
                ...
                let thresholds = thresholds.into_iter().map(|threshold| api::fee::capacity_fee::CapacityThreshold {
