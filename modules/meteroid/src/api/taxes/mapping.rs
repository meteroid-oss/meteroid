...
use meteroid_store::domain::accounting::{ProductAccounting, TaxRate, TaxRateNew, TaxRateRule};
...
/// Maps tax store errors onto the closest gRPC status so the frontend can
/// distinguish a bad request (invalid/duplicate) from a genuine failure.
pub fn tax_store_error_to_status(err: error_stack::Report<StoreError>) -> Status {
    let message = err.current_context().to_string();
    match err.current_context() {
        StoreError::InvalidArgument(_) => Status::invalid_argument(message),
        StoreError::ValueNotFound(_) => Status::not_found(message),
        StoreError::DuplicateValue { .. } => Status::already_exists(message),
        _ => Status::internal(message),
    }
}

pub fn custom_tax_new_from_server(value: server::TaxRateNew) -> Result<TaxRateNew, Status> {
    Ok(TaxRateNew {
...
pub fn tax_rule_from_server(value: server::TaxRateRule) -> Result<TaxRateRule, Status> {
...
pub fn custom_tax_to_server(value: TaxRate) -> server::TaxRate {
    server::TaxRate {
...
pub fn tax_rule_to_server(value: TaxRateRule) -> server::TaxRateRule {
    server::TaxRateRule {
