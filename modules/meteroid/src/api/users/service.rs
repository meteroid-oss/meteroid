...
impl UsersService for UsersServiceComponents {
    ...
    async fn upsert_user(
        ...
        let res = db::users::upsert_user()
            ...
            .one()
            .await
            .map_err(|e| UserApiError::DatabaseError("unable to create user".to_string(), e))?;
    ...
    async fn me(&self, request: Request<MeRequest>) -> Result<Response<MeResponse>, Status> {
        ...
        let res = db::users::get_user_by_id()
            ...
            .await
            .map_err(|e| {
                UserApiError::DatabaseEntityNotFoundError("unable to find user".to_string(), e)
            })?;
    ...
    async fn list_users(
        ...
        let res = db::users::list_users()
            ...
            .all()
            .await
            .map_err(|e| UserApiError::DatabaseError("unable to list users".to_string(), e))?;
    ...
    async fn login(
        ...
        idempotency_cache(request, |request| async {
            ...
            let user = db::users::get_user_hash_by_email()
                ...
                .await
                .map_err(|_| {
                    UserApiError::AuthenticationError("invalid email or password".to_string())
                })?;
            ...
            if user.password_hash.is_none() {
                return Err(UserApiError::AuthenticationError(
            ...
            let db_hash_parsed = PasswordHash::new(&hash_binding)
                .map_err(|_| UserApiError::InvalidArgument("password hash".to_string()))?;
    ...
    async fn register(
        ...
        idempotency_cache(request, |request| async {
            ...
            let exists = db::users::get_user_by_email()
                ...
                .opt()
                .await
                .map_err(|e| UserApiError::DatabaseError("failed to check user existence".to_string(), e))?;

            if exists.is_some() {
                return Err(UserApiError::UserAlreadyExistsError.into());
            }
            ...
            async fn create_user(
                ...
                let new_user = db::users::upsert_user()
                    ...
                    .await
                    .map_err(|e| {
                        UserApiError::DatabaseError("failed to create user".to_string(), e)
                    })?;
                ...
                let _ = db::organizations::create_organization_member()
                    ...
                    .await
                    .map_err(|e| {
                        UserApiError::DatabaseError("failed to set user role".to_string(), e)
                    })?;
            ...
            match req.invite_key {
                Some(ref invite_key) => {
                    let instance = db::organizations::get_organization_by_invite_hash()
                        ...
                        .await
                        .map_err(|e| {
                            UserApiError::DatabaseError("failed to validate invite".to_string(), e)
                        })?;
                    ...
                    transaction.commit().await.map_err(|e| {
                        UserApiError::DatabaseError("failed to commit transaction".to_string(), e)
                    })?;
                ...
                None => {
                    ...
                    let has_users = db::users::exist_users()
                        ...
                        .one()
                        .await
                        .map_err(|e| UserApiError::DatabaseError("failed to check instance users".to_string(), e))?;
                    if has_users {
                        Err(UserApiError::RegistrationClosed("registration is currently closed. Please request an invite key from your administrator.".to_string()).into())
                    } else {
                        ...
                        let org = db::organizations::create_organization()
                            ...
                            .await
                            .map_err(|e| {
                                UserApiError::DatabaseError("unable to create instance".to_string(), e)
                            })?;
                    ...
                    let _ = db::tenants::create_tenant_for_org()
                        ...
                        .await
                        .map_err(|e| {
                            UserApiError::DatabaseError("failed to create tenant".to_string(), e)
                        })?;
...
fn generate_jwt_token(user_id: &str, secret: &SecretString) -> Result<String, UserApiError> {
    let claims = Claims {
    ...
    .map_err(|e| UserApiError::JWTError("failed to generate JWT token".to_string(), e))
}

fn hash_password(password: &str) -> Result<String, UserApiError> {
    let salt = SaltString::generate(&mut OsRng);
    let argon2 = Argon2::default();
    let hash = argon2
        .hash_password(password.as_bytes(), &salt)
        .map_err(|_| UserApiError::PasswordHashingError("unable to hash password".to_string()))?;
    Ok(hash.to_string())
}
