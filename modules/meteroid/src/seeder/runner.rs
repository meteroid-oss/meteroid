...
pub async fn run(
    ...
    for customer in created_customers {
        ...
        // CHURN START
        match _plan.churn_rate {
            Some(churn_rate) => {
                if plan.plan_type != PlanTypeEnum::Free {
                    let months_since_start = (now.year() - billing_start_date.year()) * 12
                        + now.month() as i32
                    ...
                    if rng.gen::<f64>() < churn_probability {
                        ...
                        if end_date < now {
                            billing_end_date = Some(end_date);
                        }
                    }
                }
            }
    ...
    for subscription in created_subscriptions {
        ...
        for invoice_date in invoice_dates {
            ...
            let invoice = store_domain::InvoiceNew {
                ...
                amount_cents: Some(invoice_lines.iter().map(|c| c.total).sum()),
