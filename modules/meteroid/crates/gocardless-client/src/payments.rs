...
pub struct Payment {
    ...
    pub amount: i64,
    /// Cumulative refunded total in minor units; the ground truth when
    /// reconciling webhook-observed refunds.
    #[serde(default)]
...
impl PaymentApi for GoCardlessClient {
    async fn create_payment(
        ...
        // 409 idempotent_creation_conflict recovery (a prior attempt with this
        // key already created the payment) is handled inside `post_create`, so
        // a safe retry settles against the real charge instead of failing.
