use crate::domain::outbox_event::{CustomerEvent, InvoiceEvent, SubscriptionEvent};
...
pub enum PennylaneSyncRequestEvent {
    ...
    CustomerOutbox(Box<CustomerEvent>),
    /// sync event generated by an outbox event
    InvoiceOutbox(Box<InvoiceEvent>),
}
...
impl PennylaneSyncRequestEvent {
    pub fn tenant_id(&self) -> TenantId {
        match self {
            ...
            PennylaneSyncRequestEvent::InvoiceOutbox(event) => event.tenant_id,
