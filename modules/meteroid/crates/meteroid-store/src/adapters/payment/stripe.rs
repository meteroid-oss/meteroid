...
use stripe_client::payment_intents::{
    PaymentIntentApi, PaymentIntentRequest, StripeNextAction, StripePaymentIntent,
    StripePaymentStatus,
};
...
fn decline_kind_from_error(err: &Option<serde_json::Value>) -> DeclineKind {
    let Some(decline_code) = err
        .as_ref()
        .and_then(|v| v.get("decline_code").or_else(|| v.get("code")))
        .and_then(|c| c.as_str())
    else {
    ...
    Some(match (event_type, object) {
        ...
        (event_type::CHARGE_REFUNDED, EventObject::Charge(charge)) => {
            ...
            let (refund_id, refund_amount) = charge
                ...
                .map(|r| (r.id.clone(), r.amount))
                .unwrap_or_else(|| (String::new(), charge.amount_refunded));
