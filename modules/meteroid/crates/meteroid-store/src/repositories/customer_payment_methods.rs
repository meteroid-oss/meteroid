...
pub trait CustomerPaymentMethodsInterface {
    ...
    /// The provider revoked a method (mandate cancelled/expired/failed):
    /// archive the local row and clear any customer pointer to it, atomically
    /// and idempotently. Returns the method's id, or None if unknown locally.
    async fn detach_payment_method_by_external_id(
        &self,
        tenant_id: TenantId,
        external_payment_method_id: &str,
    ) -> StoreResult<Option<CustomerPaymentMethodId>>;
}
...
impl CustomerPaymentMethodsInterface for Store {
    ...
    async fn detach_payment_method_by_external_id(
        ...
        let external_id = external_payment_method_id.to_string();
        self.transaction(|conn| {
            async move {
                let Some(method) =
                    CustomerPaymentMethodRow::get_by_external_id(conn, &tenant_id, &external_id)
                        .await
                        .map_err(|err| StoreError::DatabaseError(err.error))?
                else {
                    return Ok(None);
                ...
                // Soft-archive only: historical payment_transaction rows FK it.
                CustomerPaymentMethodRow::archive(conn, &tenant_id, &method.id)
                    .await
                    .map_err(|err| StoreError::DatabaseError(err.error))?;
                CustomerRow::clear_current_payment_method(conn, tenant_id, &method.id)
                    .await
                    .map_err(|err| StoreError::DatabaseError(err.error))?;
