use crate::domain::enums::{
    BillingPeriodEnum, InvoiceStatusEnum, InvoiceType, SubscriptionFeeBillingPeriod,
};
use crate::domain::{
    CreateSubscriptionAddOns, CreateSubscriptionComponents, CreatedSubscription, Customer,
    InlineCustomer, InlineInvoicingEntity, InvoicingEntity, PriceComponent, Subscription,
    SubscriptionAddOnCustomization, SubscriptionAddOnNewInternal, SubscriptionComponentNewInternal,
...
impl SubscriptionDetails {
    pub fn calculate_cancellable_end_of_period_date(&self, now: NaiveDate) -> Option<NaiveDate> {
        ...
        let periods = crate::utils::periods::calculate_periods_for_date(
            ...
            self.subscription.billing_day_anchor,
            now,
            &period,
        );
...
impl StoreInternal {
    pub(super) async fn insert_created_outbox_events_tx(
        ...
        let outbox_events: Vec<OutboxEvent> = subscriptions
            ...
            .collect();
        self.insert_outbox_events_tx(conn, outbox_events).await
    }
}
