...
/**
 * For a given date, this gets the related arrear/advance periods
 */
...
pub fn calculate_component_period_for_invoice_date(
    ...
    let billing_period = billing_period.as_billing_period_opt();
...
fn calculate_period_idx(
    ...
    let day_adjustment =
        if considered_date.day() >= billing_day && billing_start_date.day() < billing_day {
            1
        } else {
            0
        };
