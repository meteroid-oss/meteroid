...
diesel::table! {
    ...
    invoice (id) {
        id -> Uuid,
        local_id -> Text, // local ids are unique per tenant
        invoice_number -> Text,
        invoice_type -> InvoiceType,
        reference -> Nullable<Text>, // for banking
        purchase_order -> Nullable<Text>, // optional, if org want to provide a purchase order
        //
        ...
        finalized_at -> Nullable<Timestamptz>, // eq grace_period_ended_at
        // refunded_at -> Nullable<Timestamptz>,
        invoice_date -> Date, // TODO rename
        //
        issued -> Bool,
        issue_attempts -> Int4,
        ...
        // context
        invoicing_account_id -> Uuid,
        invoicing_account_details -> Jsonb, // a snapshot of the account at the time of the invoice
        customer_id -> Uuid,
        customer_details -> Jsonb, // a snapshot of the customer at the time of the invoice
        // customer_tax_scheme -> "Auto" / "Exempt" / "ReverseCharge" / "Override" / "NotEligible",

        tenant_id -> Uuid,
        // customer_id -> Uuid,
        subscription_id -> Uuid,

        // invoicing / stripe / external
        ...
        //  Amounts are always displayed in the currency's smallest unit (most of the time, cents)
        currency -> Text,
        subtotal -> Int8, // before discounts/taxes/invoice-level minimums
        subtotal_recurring -> Int8, // part of subtotal for recurring charges

        tax_rate -> Int8,
        tax_amount -> Int8,

        total -> Int8, // after minimums/discounts/tax, before balance
        amount_due -> Int8, // after everything, remaining to pay
...
// TODO what is this ? not linked to anything
diesel::table! {
    invoicing_config (id) {
        id -> Uuid,
        tenant_id -> Uuid,
...
diesel::joinable!(invoice -> subscription (subscription_id));
