...
export const GoCardlessIntegrationModal = () => {
  ...
  const environment = useWatch({ control: methods.control, name: 'environment' })
  ...
  const steps = [
    {
      ...
      description: (
        <>
          Choose a unique alias to identify this connection, and pick the GoCardless
          environment.
          <br />
  ...
  const fieldInfo = {
    ...
    environment: {
      label: 'Environment',
      placeholder: '',
      help: 'Use sandbox until you go live.',
    },
    accessToken: {
      label: 'Access Token',
  ...
  const onSubmit = async (data: z.infer<typeof gocardlessIntegrationSchema>) => {
    try {
      await connectGoCardlessMutation.mutateAsync({
        data: {
          alias: data.alias,
