...
function radixColorKeys() {
  ...
  keys = keys.filter(key => !excludedRadixColors.some(excludeColor => key.startsWith(excludeColor)))
...
function generateColorClasses() {
  ...
  const standardColors = ['white']
  const colors = [...radixColorKeys(), ...brandColors, ...standardColors]
